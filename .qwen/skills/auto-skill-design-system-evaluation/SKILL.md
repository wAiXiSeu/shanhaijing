...
- **Tailwind v4 changes the token translation step**: `create-next-app@latest` (as of mid-2026) installs Tailwind v4, which has NO `tailwind.config.ts` — all design tokens go into `globals.css` via `@theme` blocks. When translating design spec YAML to implementation (step 5), write CSS custom properties (`--color-block-lime: #dceeb1`) inside `@theme {}` instead of a JS config file. Font-size sub-properties (line-height, letter-spacing, font-weight) use the `--text-{name}--{property}` suffix syntax. Tailwind v4 auto-generates utility classes from these tokens (e.g., `--color-block-lime` → `bg-block-lime`, `text-block-lime`).
- **Dynamic Tailwind class names break in v4 (and v3)**: Never write `bg-${variable}` — the JIT content scanner cannot detect dynamically constructed class names. Always use a static lookup mapping object: `const colors = { 'block-lime': 'bg-block-lime', ... }` and index it. This bit us in the admin categories page where color previews silently failed.
